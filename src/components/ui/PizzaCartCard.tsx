import { HTMLAttributes, ImgHTMLAttributes, useContext } from "react";
...
function PizzaDetailsCartCard({ className, children }: PizzaCardProps) {
  return (
    <div className={cn("flex  gap-5  items-center", className)}>{children}</div>
  );
}
function PizzaDetailsCartCardRight({ className, children }: PizzaCardProps) {
  return (
    <div className={cn("flex flex-col gap-4 text-gray-400 text-lg", className)}>
      {children}
    </div>
  );
}

function PizzaCartCardImage({
  className,
  children,
  src,
  ...rest
...
function PizzaCartCardFooter({ className, children }: PizzaCardProps) {
  return (
    <footer
      className={cn(
        "mt-0 flex items-center gap-12 text-2xl text-bold text-gray-500",
        className
      )}
    >
      {children}
  ...
  return (
    <div className="flex items-center gap-3  ">
      ...
      <select
        title="pizza size"
        className={cn(
          "bg-transparent border-none outline-none font-bold mt-0 text-lg text-bold text-gray-500 space-x-5",
          className
        )}
      >
        {children}
...
function PizzaCartCardSizesSelectOption({
  className,
  children,
}: PizzaCardProps) {
  return (
...
function PizzaCartCardQuantitySelect({ className, pizza }: PizzaCardProps) {
  const { addPizza, removePizza } = useContext(CartContext) as CartContextType;
  const addValue = (): void => {
    if (!pizza) return;
    addPizza(pizza, 1);
  };
